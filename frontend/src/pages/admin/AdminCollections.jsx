...
const AdminCollections = () => {
  ...
  const handleOpenAdd = () => {
    setEditingItem(null);
    setFormData({ name: '', slug: '', description: '', coverImage: '', featured: false, displayOrder: collections.length + 1, active: true });
    setModalOpen(true);
  };

  const handleOpenEdit = (col) => {
    setEditingItem(col);
    setFormData({ name: col.name, slug: col.slug, description: col.description || '', coverImage: col.coverImage || '', featured: col.featured || false, displayOrder: col.displayOrder || 0, active: col.active !== false });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingItem) {
      await API.put(`/admin/collections/${editingItem.id}`, payload);
    } else {
  ...
  return (
    <div className="space-y-6 text-left font-sans">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-obsidian-600 pb-4">
        <div>
          <h2 className="font-serif text-2xl sm:text-3xl font-bold text-gold-500 uppercase tracking-wider">Collection Management</h2>
          <p className="text-xs text-pearl-300 font-mono mt-0.5">Curate thematic collections (Bridal, Traditional, Contemporary, Festival, Daily Wear).</p>
        </div>
            ...
            <tbody className="divide-y divide-obsidian-700">
              {collections.map(c => (
                <tr key={c.id} className="hover:bg-obsidian-950/60 transition-colors">
                  ...
                  <td className="p-4 text-pearl-300">{c.featured ? 'Yes' : 'No'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-obsidian-950/80 backdrop-blur-sm">
          <div className="bg-obsidian-900 border border-gold-500/40 shadow-2xl w-full max-w-md p-6 space-y-4 text-left font-mono text-xs">
            <div className="flex justify-between border-b border-obsidian-600 pb-3">
              <h3 className="font-serif text-lg font-bold text-gold-400 uppercase">{editingItem ? 'Edit Collection' : 'Create Collection'}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <label className="flex items-center gap-2 text-xs font-bold text-pearl-200 cursor-pointer">
                <input type="checkbox" checked={formData.featured} onChange={e => setFormData({ ...formData, featured: e.target.checked })} className="accent-gold-500" /> Featured Collection
              </label>
