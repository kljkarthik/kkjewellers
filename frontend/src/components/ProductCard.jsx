    ...
    <div className="group bg-obsidian-900 hover:bg-obsidian-700 rounded-none overflow-hidden border border-obsidian-600 hover:border-gold-500/80 transition-all duration-500 flex flex-col h-full relative text-left">
      ...
      <div className="relative aspect-square overflow-hidden bg-obsidian-950">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10">
          {product.newArrival && (
            <span className="bg-obsidian-950/90 text-gold-400 text-[9px] uppercase font-mono font-bold tracking-widest px-2.5 py-1 border border-gold-500/40">
              New Arrival
            </span>
          )}
          {product.featured && (
            <span className="bg-gold-500 text-obsidian-950 text-[9px] uppercase font-mono font-bold tracking-widest px-2.5 py-1 flex items-center gap-1">
        ...
        <button
          ...
          className={`absolute bottom-3 right-3 z-20 p-2.5 rounded-full border transition-all ${
            wishlisted
        ...
        >
          <Heart className={`w-4 h-4 transition-transform ${wishlisted ? 'fill-obsidian-950 scale-110' : ''}`} />
        </button>

        {/* Material Tag */}
        {product.material && (
          <span className="absolute top-3 right-3 bg-obsidian-950/90 text-pearl-300 text-[10px] font-mono tracking-wider px-2.5 py-1 border border-gold-500/30">
      ...
      {/* Editorial Details */}
      <div className="p-5 flex flex-col flex-grow bg-obsidian-900 group-hover:bg-obsidian-700 transition-colors border-t border-obsidian-600 relative">
        <div className="flex items-center justify-between text-[11px] font-mono text-gold-500 mb-1">
        ...
        <Link to={`/product/${product.productCode}`}>
          <h3 className="font-serif text-lg font-normal text-pearl-100 group-hover:text-gold-400 transition-colors line-clamp-1 mb-2 tracking-wide uppercase">
            {product.name}
          </h3>
        </Link>

        <p className="text-pearl-300 text-xs line-clamp-2 mb-4 font-light leading-relaxed flex-grow">
          {product.shortDescription || product.fullDescription}
        </p>

        {/* Card Footer: VIEW PIECE → */}
        <div className="pt-3 border-t border-obsidian-600 flex items-center justify-between relative">
          <span className="text-[10px] uppercase font-mono tracking-widest text-pearl-300">
            {product.collection?.name || 'Heritage Collection'}
          </span>
          <Link
            to={`/product/${product.productCode}`}
            className="text-xs font-bold text-gold-500 hover:text-gold-400 uppercase tracking-widest flex items-center gap-1.5 transition-all group-hover:translate-x-2 duration-300"
          >
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-obsidian-950/80 backdrop-blur-sm">
          <div className="bg-obsidian-900 border border-gold-500/50 p-6 max-w-sm w-full text-center space-y-4 shadow-2xl">
            <Heart className="w-10 h-10 text-gold-500 mx-auto" />
            <h4 className="font-serif text-xl font-bold text-gold-400 uppercase">SIGN IN TO SAVE THIS PIECE</h4>
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button
                ...
                className="w-full py-3 bg-gold-500 text-obsidian-950 font-bold text-xs uppercase tracking-widest shadow-obsidian-glow"
              >
                SIGN IN
              </button>
              <button
                onClick={() => { setShowAuthModal(false); navigate('/register'); }}
                className="w-full py-3 bg-obsidian-950 text-pearl-100 border border-gold-500/30 text-xs font-bold uppercase tracking-widest"
              >
                CREATE ACCOUNT
              </button>
              <button
                onClick={() => setShowAuthModal(false)}
                className="text-[11px] text-pearl-300 hover:underline pt-1 font-mono"
              >
                Continue Browsing
              </button>
            </div>
